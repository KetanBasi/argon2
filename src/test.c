...
void hashtest(uint32_t version, uint32_t t, uint32_t m, uint32_t p, char *pwd,
    ...
    printf("Hash test: $v=%d t=%d, m=%d, p=%d, pass=%s, salt=%s: ", version, t,
           m, p, pwd, salt);

    ret = argon2_hash(t, 1 << m, p, pwd, strlen(pwd), salt, strlen(salt), "", 0,
                      out, OUT_LEN, encoded, ENCODED_LEN, type, version);
    ...
    ret = argon2_verify(encoded, pwd, strlen(pwd), "", 0, type);
    assert(ret == ARGON2_OK);
    ret = argon2_verify(mcfref, pwd, strlen(pwd), "", 0, type);
    assert(ret == ARGON2_OK);
...
int main() {
    ...
    ret = argon2_verify("$argon2i$m=65536,t=2,p=1c29tZXNhbHQ"
                        "$9sTbSlTio3Biev89thdrlKKiCaYsjjYVJxGAL3swxpQ",
                        "password", strlen("password"), "", 0, Argon2_i);
    ...
    ret = argon2_hash(2, 1, 1, "password", strlen("password"), "diffsalt",
                      strlen("diffsalt"), "", 0, out, OUT_LEN, NULL, 0,
                      Argon2_id, version);
    assert(ret == ARGON2_MEMORY_TOO_LITTLE);
    printf("Fail on invalid memory: PASS\n");

    ret = argon2_hash(2, 1 << 12, 1, NULL, strlen("password"), "diffsalt",
                      strlen("diffsalt"), "", 0, out, OUT_LEN, NULL, 0,
                      Argon2_id, version);
    ...
    ret = argon2_hash(2, 1 << 12, 1, "password", strlen("password"), "s", 1, "",
                      0, out, OUT_LEN, NULL, 0, Argon2_id, version);
    assert(ret == ARGON2_SALT_TOO_SHORT);
